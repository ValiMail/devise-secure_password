...
RSpec.describe Devise::Models::PasswordRegularUpdateEnforcement, type: :model do
  let(:password) { 'Bubb1234@#$!' }
  let(:user) do
    Isolated::UserRegularUpdate.new(
  ...
  describe 'config' do
    subject { Isolated::UserRegularUpdate }
    ...
    context 'when password_frequent_change_prevention module is not enabled' do
      it 'raises a ConfigurationError' do
        expect do
          Isolated::UserRegularUpdateBad.new
        end.to raise_error(Devise::Models::PasswordRegularUpdateEnforcement::ConfigurationError)
      end
    end
    ...
    context 'when config.password_maximum_age is invalid' do
      it 'raises a ConfigurationError' do
        expect do
          Isolated::UserRegularUpdateBadConfig.new
        end.to raise_error(Devise::Models::PasswordRegularUpdateEnforcement::ConfigurationError, /invalid type/)
      end
    end
  end
  ...
  describe '#password_expired?' do
    ...
    context 'when password has expired' do
      before do
        ...
        last_password.created_at = Time.zone.now - Isolated::UserRegularUpdate.password_maximum_age
        last_password.save
      end
